...
The last extractor is [`discourse_topic`](./src/extract/extractors/discourse_topic.py) which specifically extracts the contents of a topic from a [Discourse](https://discourse.org) forum. I made this extractor because I used my modding [forum](https://forum.moddingcommunity.com) to test this project.

Additional extractors may be created for those interested! Feel free to take a look at the source code from [`src/extract`](./src/extract/).

As of right now, there are two types of extractors 

the only websites this project can extract data from are topics from Discourse forums (using the [`discourse_topic`](./src/extract/extractors/discourse_topic.py) extractor) since that's what I used when testing this application. You may create your own extractors as well, please take a look at the source code [here](./src/extract/) for more information.
